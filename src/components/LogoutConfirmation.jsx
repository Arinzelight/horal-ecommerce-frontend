...
const LogoutConfirmation = () => {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      setLoading(false);
      setTimeout(() => dispatch(closeLogoutModal()), 100);
    }
  };

  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-indigo-50 rounded-lg p-6 flex flex-col items-center gap-6 shadow-xl">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-base font-medium text-neutral-900 text-center">
            Are you sure you want to log out of your Horal account?
          </p>
        </div>
        <div className="flex flex-row gap-4 w-full justify-center">
          <button
            ...
            disabled={loading}
            className="flex-1 px-4 py-2 bg-white rounded hover:opacity-90 text-stone-900 text-sm font-bold shadow"
          >
            Cancel
          </button>
          <button
