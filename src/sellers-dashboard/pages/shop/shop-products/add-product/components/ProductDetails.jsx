...
const ProductDetails = ({ formData, onInputChange, selectedCategory }) => {
  ...
  const isAtLimit = titleLength >= 240;
      ...
      <div className="space-y-4 border-[1px] border-neutral-200 p-4 rounded-md">
        <div>
          ...
          <div className="relative">
            {/* Change title field to a text area */}
            <textarea
              id="title"
              required
              value={formData.title}
              onChange={(e) => handleTitleChange(e.target.value)}
              className={`w-full h-24 px-3 py-2 border rounded-md focus:outline-none focus:ring-1 pr-12 ${
                isAtLimit
          ...
          {titleLength > 40 && (
            ...
            >
              {isAtLimit
                ...
                : `${240 - titleLength} characters remaining`}
            </p>
          )}
        </div>
